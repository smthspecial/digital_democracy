...
- A cross-service e2e test lives inside the TS service that owns one side of the flow, under that service's own `src/e2e/` directory (`services/ts/identity-service/src/e2e/harness.ts` is the canonical `harness.ts` every other `src/e2e/` copies, per its own header comment on why it's duplicated rather than shared). When a flow spans two or more **Go** services, or spans enough services that no single one is the obvious host, it goes in the standalone `testing/e2e-suite` package instead (e.g. `arch019-delegated-expertise.e2e.test.ts`, covering delegation-service + voting-service, neither of which has a TS home). Same harness conventions apply either way: every service under test is spawned as its own real process (`tsx` for TS, a built binary for Go) on a fixed local port, reached over real HTTP — nothing mocks another service's business logic. Test file names and scenario ids (`HPn`/`ECn`) match the `.spec/technical/architecture/arch-NNN.md` doc they cover, verbatim, so a failing test traces back to that doc directly. Run with `pnpm test` (or `pnpm --filter @dd/e2e-suite test` from the repo root for the standalone package).
...
[TP-001](../test-plans/tp-001.md) is a companion, not one of the twelve: a single, code-grounded, service-by-service API test plan spanning all 18 services at once (happy paths, edge cases, and an adversarial "criminal user" abuse dimension the flow docs above don't systematically cover), using its own separate `HP-N`/`EDGE-<SVC>-N`/`ABUSE-<SVC>-N` numbering. Where a scenario in TP-001 has a real automated test, the corresponding ARCH-010..022 flow doc's traceability table carries the test id and cross-references TP-001's scenario id — see TP-001 §6 for the current list.
