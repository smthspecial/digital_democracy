...
- **Where the target of a seam has no real callable HTTP surface at all yet** (for example, an asynchronous consumer relationship modeled only as a stub because no message queue exists in this codebase — ARCH-004/ADR-016 describe the eventual Kafka backbone, not something implemented here), the flow doc still documents the scenario and marks it `blocked on: <what's missing>` rather than silently assuming it works.
