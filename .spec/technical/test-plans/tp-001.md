...
id: TP-001
type: test-plan
title: "Cross-service API test plan: happy paths, edge cases & abuse scenarios"
status: deprecated
testScope: e2e
linkedIds: ARCH-009,ARCH-013,ARCH-015,ARCH-016,ARCH-017,ARCH-019,ARCH-020,ARCH-021,ARCH-022
created: 2026-09-04
---

> **Historical** (ADR-033): written pre-ADR-027 pivot for an 18-microservice topology that no longer exists (now `apps/api-go` + `apps/api-ts`). The **`ABUSE-<SVC>-N` adversarial-scenario catalog remains this repo's most valuable source of security test cases** and should be ported against the current two-app topology (CC-15's salvage inventory, ARCH-026, is the reference for what moved where) rather than rewritten from scratch — the attack itself (double-submit, replay, cross-jurisdiction escalation, etc.) doesn't depend on which process boundary it crosses. `VULNERABLE` verdicts below are unverified against current code; re-confirm each before filing as a live bug. HP-N/EC-N scenario ids remain valid citations.
...
**Relationship to ARCH-009 and the ARCH-010..022 flow docs**: ARCH-009 defines the integration/e2e test *strategy* (levels, tooling, taxonomy, `HP-N`/`EC-N` naming per flow doc); ARCH-010 through ARCH-022 enumerate flow-specific scenarios under that convention, one doc per citizen-facing journey. This doc is a different, complementary artifact: a single, code-grounded, service-by-service API test plan spanning all 18 services at once, including the adversarial/abuse dimension the flow docs don't systematically cover. Its `HP-N`/`EDGE-<SVC>-N`/`ABUSE-<SVC>-N` scenario ids are a **separate, service-grouped numbering** from the flow docs' flow-grouped `HP-N`/`EC-N` — where a scenario here has been implemented as a real automated test, the owning flow doc's traceability table carries the real test id and cross-references the id used here (see ARCH-013 EC-43, ARCH-015 EC-39, ARCH-016 EC-18, ARCH-017 EC-16, ARCH-019 EC-27, ARCH-021 EC-47/EC-52 for the six scenarios implemented so far). This doc is the source catalog those tests were written against; it is not restated or duplicated there.

This document replaces the standalone `testing/e2e-api-test-plan.md` file, which has been removed — per this project's convention, test plans and testing strategy live in `.spec/`, not in scattered repo-root docs.
...
**Expected**: chain resolution, weighting, and revocation all behave per §domain rules. See ABUSE-DELEG-* for the same flow's adversarial variant. Implemented as real e2e tests — see ARCH-019 §"Status update (2026-09-04)" and `testing/e2e-suite/src/e2e/arch019-delegated-expertise.e2e.test.ts`.

**Also implemented, beyond this doc's original scope**: HP-1 (identity+auth onboarding) has a full real e2e suite at `services/ts/identity-service/src/e2e/arch010-identity-lifecycle.e2e.test.ts`, covering this scenario plus most of ARCH-010's edge cases (see ARCH-010 §5). HP-3 (jurisdiction/residency/eligibility) and the problem→proposal portion of HP-4 are covered by `services/ts/proposal-service/src/e2e/arch011-jurisdiction-scope.e2e.test.ts` and `arch012-problem-proposal.e2e.test.ts` respectively (see ARCH-011/ARCH-012 §5) — the full HP-4 chain (through voting/project/outcome) remains unimplemented past the problem→proposal segment. HP-7 (IAM dual-control) has a full real e2e suite at `services/ts/iam-service/src/e2e/policy-lifecycle.e2e.test.ts` (no ARCH-010..022 doc owns iam-service; ARCH-024 is a design doc, not a test plan — this is TP-001's only record of that suite).
...
| EDGE-BUD-7 | Negative ledger amount | `POST /budget/ledger {type:"inflow", amount:-500}` | **201 — no sign/positivity validation.** Confirms the public ledger can be corrupted with negative entries. Implemented — see ARCH-015 EC-39 / ABUSE-FIN-2 below. |
...
| EDGE-PROJ-6 | Overspend | `budget-spent` cumulative amount exceeds `budget_allocated` | **200 — no ceiling check exists.** Confirms the gap explicitly (also see ABUSE-FIN-2). Implemented — see ARCH-017 EC-16 / ABUSE-FIN-3 below. |
...
| ABUSE-VOTE-3 | Ballot stuffing via `/open`'s caller-supplied eligible list | Call `POST /voting/sessions/{id}/open {eligible_citizen_ids: [...100 fabricated ids...]}` | DP-025: eligibility should be read from jurisdiction-service, independently verified | Code takes the array directly from the request body with **zero cross-check** against jurisdiction-service | **VULNERABLE** — confirmed top-priority finding; whoever can call `/open` controls the electorate outright. **Implemented**: `services/go/voting-service/abuse_test.go::TestAbuseVote3And4OpenAcceptsFabricatedElectorateWithNoJurisdictionCheck` (see ARCH-016 EC-18). |
| ABUSE-VOTE-4 | Disenfranchisement via `/open` omission | Same endpoint, omit specific legitimate citizen IDs from the list | All eligible citizens should receive a token | Same root cause as ABUSE-VOTE-3 — omission is just as easy as injection | **VULNERABLE** — proven by the same test as ABUSE-VOTE-3 (see ARCH-016 EC-18). |
...
| ABUSE-FIN-2 | Negative-amount ledger corruption | `POST /budget/ledger {type:"outflow", amount:-50000}` | Amounts should be positive; sign is implied by `type` | No sign/positivity validation anywhere | **VULNERABLE** — directly corrupts reconciliation math. **Implemented**: `services/ts/budget-service/src/routes/ledger.test.ts`'s two `ABUSE-FIN-2` cases (see ARCH-015 EC-39). |
| ABUSE-FIN-3 | Project overspend / budget drain | Repeated `POST /:id/budget-spent` calls on a project well past its `budget_allocated` | Spend should be capped at allocation | No ceiling check in `recordBudgetSpent` | **VULNERABLE** — **Implemented**: `services/ts/project-service/src/routes/projects.test.ts::"ABUSE-FIN-3: records spend far beyond budget_allocated with no ceiling check"` (see ARCH-017 EC-16). |
...
| ID | Scenario | Steps | Spec says | Code does | Verdict |
|---|---|---|---|---|---|
| ABUSE-REP-1 | Direct self-boosting | `POST /reputation/records {citizen_id: self, factor_type:"successful_proposal", delta:+1000000, source_ref:"anything"}` repeated | DP-038: reputation changes should only originate from authorized upstream services following a real event | No caller-authentication on this endpoint at all; `source_ref` only checked for non-emptiness, not authenticity | **VULNERABLE** — the single largest reputation-integrity finding; unbounded, unverified, directly callable. **Implemented**: `services/ts/reputation-service/src/reputation.test.ts`'s `"ABUSE-REP-1/2"` suite (see ARCH-013 EC-43). |
| ABUSE-REP-2 | Rival defamation | `POST /reputation/records {citizen_id: rival, factor_type:"fraud", delta:-1000, source_ref:"fabricated"}` | Negative deltas require "an upstream authoritative decision" (srv-014.md) | Same as above — only sign/non-empty-string checks exist | **VULNERABLE** — proven by the same suite as ABUSE-REP-1 (see ARCH-013 EC-43). |
...
| ID | Scenario | Steps | Spec says | Code does | Verdict |
|---|---|---|---|---|---|
| ABUSE-AUDIT-1 | Forged actor_ref | `POST /audit/log {action_type:"vote_certified", actor_ref:"voting-service", payload:{fabricated}}` from an arbitrary caller, not the real voting-service | The audit trail should reflect true system events from authenticated services | No caller authentication; `actor_ref` is a self-declared string; the hash chain proves **sequence** integrity, not **authorship** | **VULNERABLE** — a forged entry passes `/verify` as fully valid, since the chain math never asserted authenticity in the first place. **Implemented**: `services/go/audit-service/abuse_test.go::TestAbuseAudit1ForgedActorRefPassesChainVerification` (see ARCH-021 EC-52). |
| ABUSE-AUDIT-2 | Ballot-content leakage into the public log | Submit a `payload` on any audit event that includes actual ballot-choice content | NFR-001: ballot content must never be logged | Enforced entirely by caller discipline; audit-service performs no payload content filtering itself | **VULNERABLE (structural)** — confirms this is enforced by convention, not by the service, exactly as ARCH-021 EC-51 flags |
| ABUSE-AUDIT-3 | Constitutional-review evasion by rephrasing | Register a protected right named e.g. `"due_process"`; submit a `change_summary` that clearly violates it in substance but never uses that literal string (use a synonym or paraphrase) | DP-034 should catch substantive violations | `keywordMatchAssessor` is a literal case-insensitive substring match on the right's name only | **VULNERABLE** — trivially evaded by rewording. **Implemented**: `services/go/audit-service/abuse_test.go::TestAbuseAudit3ConstitutionalReviewEvadedByRephrasing` (see ARCH-021 EC-47). |
...
2. **`voting-service`'s `/voting/sessions/{id}/open` trusts a caller-supplied elector list with zero independent eligibility verification** (ABUSE-VOTE-3/4, now regression-tested — see ARCH-016 EC-18) — whoever can call this endpoint controls the outcome of every election outright.
...
4. **`reputation-service` and `audit-service` accept direct, unauthenticated writes** (ABUSE-REP-1/2, now regression-tested — see ARCH-013 EC-43; ABUSE-AUDIT-1, now regression-tested — see ARCH-021 EC-52) — both are meant to be authoritative, tamper-evident records; today anyone can write to either directly.
5. **Financial integrity gaps in budget/project services** (ABUSE-FIN-1 through 8, two now regression-tested — see ARCH-015 EC-39 and ARCH-017 EC-16) — fabricated ledger entries, unbounded overspend, no idempotency.
...
7. **Constitutional-review and protocol-gate logic is trust-the-caller** (ABUSE-GOV-2/3, ABUSE-AUDIT-3 — the latter now regression-tested, see ARCH-021 EC-47) — the checks that are supposed to prevent rushed/rights-violating protocol changes are currently either keyword-based or fully client-asserted.
...
Seven scenarios from this catalog have real, passing automated tests today, each cross-referenced from its owning ARCH-0NN flow doc's traceability table:

| Scenario | Test | Owning flow doc |
|---|---|---|
| ABUSE-AUDIT-1 | `services/go/audit-service/abuse_test.go::TestAbuseAudit1ForgedActorRefPassesChainVerification` | ARCH-021 EC-52 |
| ABUSE-AUDIT-3 | `services/go/audit-service/abuse_test.go::TestAbuseAudit3ConstitutionalReviewEvadedByRephrasing` | ARCH-021 EC-47 |
| ABUSE-IDOR-4 | `services/go/delegation-service/abuse_test.go::TestAbuseIDOR4RevokeAuthorizedByPubliclyReadableDelegatorID` | ARCH-019 EC-27 |
| ABUSE-VOTE-3/4 | `services/go/voting-service/abuse_test.go::TestAbuseVote3And4OpenAcceptsFabricatedElectorateWithNoJurisdictionCheck` | ARCH-016 EC-18 |
| ABUSE-FIN-2 | `services/ts/budget-service/src/routes/ledger.test.ts` (2 cases) | ARCH-015 EC-39 |
| ABUSE-FIN-3 | `services/ts/project-service/src/routes/projects.test.ts::"ABUSE-FIN-3: ..."` | ARCH-017 EC-16 |
| ABUSE-REP-1/2 | `services/ts/reputation-service/src/reputation.test.ts` (2 cases) | ARCH-013 EC-43 |

Beyond the seven abuse scenarios above, several HP scenarios also have real coverage via the ARCH-010..022 flow docs' own (more detailed) e2e suites, not originally tracked here:

| Scenario | Test | Owning flow doc |
|---|---|---|
| HP-1 (identity+auth) | `services/ts/identity-service/src/e2e/arch010-identity-lifecycle.e2e.test.ts` (HP-1..4 + most EC's) | ARCH-010 §5 (fully synced) |
| HP-3 (jurisdiction/eligibility) | `services/ts/proposal-service/src/e2e/arch011-jurisdiction-scope.e2e.test.ts` | ARCH-011 §5 (fully synced) |
| HP-4 (partial — problem→proposal segment only) | `services/ts/proposal-service/src/e2e/arch012-problem-proposal.e2e.test.ts` | ARCH-012 §5 (fully synced) |
| HP-5 (delegation+voting) | `testing/e2e-suite/src/e2e/arch019-delegated-expertise.e2e.test.ts` | ARCH-019 §5 |
| HP-7 (IAM dual control) | `services/ts/iam-service/src/e2e/policy-lifecycle.e2e.test.ts` | none (ARCH-024 is a design doc, not a test plan — tracked here only) |

ARCH-010, ARCH-011, and ARCH-012 are themselves fully traceability-synced (every HP/EC row has a real test id or an explicit `blocked on:` note) — this doc's own EDGE-ID-*/EDGE-AUTH-*/EDGE-JUR-*/EDGE-PRB-*/EDGE-PRP-* catalogs are the *lighter*, service-grouped restatement of what those three docs cover in much greater depth; treat ARCH-010/011/012 as authoritative for those four services, not this section.

Genuinely unimplemented, zero real automated coverage as of 2026-09-05: ARCH-014 (deliberation, 42 scenarios), ARCH-018 (civic-duty, 40 scenarios), ARCH-020 (constitutional layer, 33 scenarios), and 46 of ARCH-022's 112 scenarios (platform reliability — the other 66 are already covered, largely by auth-service's and identity-service's own existing suites). Every EDGE-*/ABUSE-* scenario in §2/§3 above for competency-service, deliberation-service, civic-duty-service, notification-service, ai-synthesis-service, governance-role-service, and auth-service beyond what ARCH-010/022 already cover remains a documented scenario, not yet an automated test.
