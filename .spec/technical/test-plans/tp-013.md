...
- **EC-47.** Constitutional review's placeholder `keywordMatchAssessor` is a case-insensitive substring match of a protected right's `name` against the caller-supplied `change_summary` — trivially over-triggered (any unrelated mention of the right's name blocks the change) and trivially under-triggered (a paraphrase that never uses the name clears it). Document current behavior as-implemented; `service.go` itself comments this is "a placeholder trigger only." Cites: FR-060, FR-067. Level: integration. Proven (the under-triggered half) by `abuse_test.go::TestAbuseAudit3ConstitutionalReviewEvadedByRephrasing` (ABUSE-AUDIT-3, TP-001): a change summary that plainly bans public criticism of officials — a substantive free-speech violation — sails through uncaught because it never uses the literal words "freedom" or "speech".
- **EC-52** (ABUSE-AUDIT-1, TP-001): `POST /audit/log`'s `actor_ref` is a self-declared string with no caller authentication behind it — the hash chain proves entry *sequence* is unbroken, not entry *authorship*. A forged entry claiming `actor_ref: "voting-service"` for a `vote_certified` event that service never actually certified is accepted (`201`), lists identically to a genuine entry, and `GET /audit/log/verify` still reports `valid: true` afterward — the chain's guarantee is narrower than "nothing false was ever recorded here." Cites: FR-060, DP-036. Level: integration. Proven by `abuse_test.go::TestAbuseAudit1ForgedActorRefPassesChainVerification`.
...
| EC-47 | FR-060,FR-067 | integration | `abuse_test.go::TestAbuseAudit3ConstitutionalReviewEvadedByRephrasing` |
...
| EC-52 | FR-060,DP-036 | integration | `abuse_test.go::TestAbuseAudit1ForgedActorRefPassesChainVerification` |
