...
status: deprecated
testScope: e2e
linkedIds: ARCH-022,EPIC-013,NFR-009,NFR-011,AUTH-011,AUTH-012,AUTH-010,AUTH-006,ADR-001,ADR-014,ADR-015,ADR-016,ADR-017,ADR-018,ARCH-006,ARCH-007,ARCH-008,ARCH-009,DP-066,DP-067,DP-068,SRV-017
created: 2026-08-27
---
...
Moved from `ARCH-022` (type `arch`) to this doc (`TP-014`, type `test-plan`) — test-plan content belongs under the `test-plan` type, not `arch`. `ARCH-022` is now a deprecated stub pointing here.

**Correction to an earlier claim made in conversation, not in this doc:** every single-service scenario testable today (HP-1 through EC-49, all auth-service/governance-role-service) shows `TBD` in the Automated test id column above — **none** of them have a real automated test yet, despite `auth-service` and `governance-role-service` both having extensive existing unit suites for other things. An earlier progress summary in this session claimed "46 of 112 still open... 66 already covered" for this doc; that count was a measurement error (a grep matched both this doc's §3 scenario-definition tables, which have no test-id column at all, and its real §5 traceability table, inflating the apparent "total" and making untested rows look covered). The correct current state is: zero of this doc's ~64 distinct scenarios (9 HP + 49 EC in §3, plus 2 HP + 6 EC forward-looking in §4) have a real test, and the 6 forward-looking §4 scenarios remain acceptance criteria with no infrastructure to run them against at all.
