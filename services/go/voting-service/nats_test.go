...
func TestNATSAuditEmitterPublishesVoteCertified(t *testing.T) {
	...
	if err := emitter.Emit("vote_certified", "session-1"); err != nil {
		t.Fatalf("Emit: %v", err)
	}
	...
	select {
	case msg := <-received:
		if msg["action_type"] != "vote_certified" {
			t.Errorf("action_type = %v, want vote_certified", msg["action_type"])
		}
		if msg["actor_ref"] != "voting-service" {
			t.Errorf("actor_ref = %v, want voting-service", msg["actor_ref"])
		}
		if msg["payload"] != "session-1" {
			t.Errorf("payload = %v, want session-1", msg["payload"])
		}
