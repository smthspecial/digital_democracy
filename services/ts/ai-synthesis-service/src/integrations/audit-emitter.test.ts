...
import { AUDIT_APPEND_STREAM, AUDIT_APPEND_SUBJECT, createNatsAuditEmitter } from "./audit-emitter.js";
...
describe("createNatsAuditEmitter (integration, against a real nats-server)", () => {
  ...
  async function startNatsServer(): Promise<string> {
    ...
    const storeDir = await mkdtemp(path.join(tmpdir(), "dd-ai-synthesis-nats-"));
  ...
  it("IT-eventbus: publishes system_update with the synthesis event as payload", async () => {
    ...
    emitter.emit({ type: "ai_synthesis.executed", proposalId: "p1", outputId: "out-1", occurredAt });
    ...
    expect(received).toMatchObject({
      ...
      payload: {
        event_type: "ai_synthesis.executed",
        proposalId: "p1",
        outputId: "out-1",
