...
import { AUDIT_APPEND_STREAM, AUDIT_APPEND_SUBJECT, createNatsAuditEmitter } from "./integrations.js";
...
describe("createNatsAuditEmitter (integration, against a real nats-server)", () => {
  ...
  async function startNatsServer(): Promise<string> {
    ...
    const storeDir = await mkdtemp(path.join(tmpdir(), "dd-project-nats-"));
  ...
  it("IT-eventbus: publishes system_update with the original event_type folded into the payload", async () => {
    ...
    emitter.emit({ type: "project.completed", projectId: "proj-1", at });
    ...
    expect(received).toMatchObject({
      ...
      payload: { event_type: "project.completed", projectId: "proj-1", at: at.toISOString() },
    });
  });

  it("IT-eventbus: folds details into the payload when present", async () => {
    const url = await startNatsServer();
    bus = await connectEventBus(url);
    await ensureStream(bus, { name: AUDIT_APPEND_STREAM, subjects: [AUDIT_APPEND_SUBJECT] });
    ...
    expect(received).toMatchObject({
      payload: { event_type: "project_milestone.completed", details: { milestoneId: "m1" } },
    });
  });
});
